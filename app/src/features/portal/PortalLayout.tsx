...
export default function PortalLayout() {
  ...
  const nombre = user?.displayName?.split(" ")[0] || (locale === "es" ? "cliente" : "friend");
      ...
      <header className="sticky top-0 z-40 border-b border-gray-200 bg-white">
        <div className="container-app flex h-16 items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Brand to="/portal" ariaLabel={t("portal.badge")} />
          ...
          <div className="flex items-center gap-3">
            <span className="hidden text-sm text-gray-500 sm:inline">
              {t("portal.hello", { nombre })}
            </span>
            <Link to="/solicitar" className="hidden sm:block">
              <Button variant="gold" size="sm">
                <Plus className="h-4 w-4" /> {t("portal.request")}
              </Button>
            </Link>
